...
namespace Core.Tests.API.Controllers.Security
{
    ...
    public class MessageControllerTests
    {
        ...
        public MessageControllerTests()
        {
            ...
            _controller = new MessageController(_coreContext);
        }
        ...
        {
            // arrange
            var expectedEntities = _coreContext.Messages.OrderBy(p => p.Id).ToList();
            // act
        ...
        {
            // arrange
            var expectedEntity = _coreContext.Messages.FirstOrDefault();
            // act
        ...
        public void PostTest()
        {
            ...
            // act
            var actualIDs = _controller.Post(new List<Message> { entityToAdd });
            // assert
            var actualEntity = _coreContext.Messages.FirstOrDefault(p => p.Id == actualIDs.ToList()[0]);
            if (actualEntity == null) Assert.Fail("actualEntity was null");
            Assert.AreEqual(actualIDs.ToList()[0], actualEntity.Id);
            //cleanup
            _coreContext.Messages.Remove(actualEntity);
        ...
        public void PutTest()
        {
            ...
            _coreContext.Messages.Add(entityToUpdate);
            _coreContext.SaveChanges();
            var expectedEntity = _coreContext.Messages.OrderByDescending(p => p.LastUpdatedUtc).FirstOrDefault();
            // act
            if (expectedEntity == null) Assert.Fail("expectedEntity was null");
        ...
        public void DeleteTest()
        {
            ...
            _coreContext.Messages.Add(entityToDelete);
            _coreContext.SaveChanges();
            var actualEntity = _coreContext.Messages.OrderByDescending(p => p.LastUpdatedUtc).FirstOrDefault();
