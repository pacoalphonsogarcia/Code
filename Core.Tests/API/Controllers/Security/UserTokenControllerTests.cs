...
namespace Core.Tests.API.Controllers.Security
{
    ...
    public class UserTokenControllerTests
    {
        ...
        private readonly UserTokenController _controller;

        public UserTokenControllerTests()
        {
            _coreContext = TestHelper.GetDatabase();
            _controller = new UserTokenController(_coreContext);
        }
        ...
        {
            // arrange
            var expectedEntities = _coreContext.UserTokens.OrderBy(p => p.Id).ToList();
            // act
        ...
        {
            // arrange
            var expectedEntity = _coreContext.UserTokens.FirstOrDefault();
            // act
        ...
        public void PostTest()
        {
            ...
            // act
            var actualIDs = _controller.Post(new List<UserToken> { entityToAdd });
            // assert
            var actualEntity = _coreContext.UserTokens.FirstOrDefault(p => p.Id == actualIDs.ToList()[0]);
            if (actualEntity == null) Assert.Fail("actualEntity was null");
            Assert.AreEqual(actualIDs.ToList()[0], actualEntity.Id);
            //cleanup
            _coreContext.UserTokens.Remove(actualEntity);
        ...
        public void PutTest()
        {
            ...
            _coreContext.UserTokens.Add(entityToUpdate);
            _coreContext.SaveChanges();
            var expectedEntity = _coreContext.UserTokens.OrderByDescending(p => p.LastUpdatedUtc).FirstOrDefault();
            // act
            if (expectedEntity == null) Assert.Fail("expectedEntity was null");
        ...
        public void DeleteTest()
        {
            ...
            _coreContext.UserTokens.Add(entityToDelete);
            _coreContext.SaveChanges();
            var actualEntity = _coreContext.UserTokens.OrderByDescending(p => p.LastUpdatedUtc).FirstOrDefault();
        ...
        private UserToken CreateEntity()
        {
            var newEntity = new UserToken
            {
